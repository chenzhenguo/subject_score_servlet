    ...
    <div class="container-fluid">
        ...
        <div class="row-fluid">
            <div class="span3">
                ...
                <ul id="dashboard-menu" class="nav nav-list collapse in">
                  ...
                  <li  class="active"><a href="points">积分管理</a></li>
                </ul>
            </div>
        </div>
        <div class="span9">
     ...
     <h1 class="page-title">积分管理</h1>
			<div class="well search-well">
                <form id="searchForm" class="form-inline" action="pnt" method="post">
                    <input class="input-mini" placeholder="学生姓名" name="name" type="text" value="${pb.name }">
                    <input class="input-mini" placeholder="年龄" name="age" type="text" value="${pb.age }">
                    <input class="input-small" placeholder="班级" name="className" type="text" value="${pb.className }">
                    <input class="input-small" placeholder="电话" name="phone" type="text" value="${pb.phone }">
                    <input class="input-small" placeholder="专业" name="mayor" type="text" value="${pb.mayor }">
                    <input class="input-small" placeholder="学历" name="education" type="text" value="${pb.education }">
                      <input class="input-small" placeholder="积分" name="score" type="text" value="${pb.score }">
                    <input class="input-small" placeholder="入学时间" name="hireDate" type="text" value="${pb.hireDate }">
...
<div class="well">
    <table class="table">
      ...
      <tbody>
        ...
        <tr>
          <td>${bean.student.id }</td>
          <td>${bean.student.name }</td>
          <td>${bean.student.age }</td>
          <td>
          	<c:if test="${bean.student.sex==0 }">女</c:if>
          	<c:if test="${bean.student.sex==1 }">男</c:if>
          </td>
          <td>${bean.student.idCard }</td>
          <td>${bean.student.phone }</td>
          <td>${bean.student.mayor }</td>
          <td>${bean.student.status.name }</td>
          <td>${bean.student.education } </td>
          <td>${bean.score } </td>
          <td>${bean.student.school }</td>
          <td>${bean.student.address }</td>
          <td>${bean.student.hireDate }</td>
          <td>
...
<div class="pagination">
    ...
    <ul>
        <li><a href="javascript:pageFunction('points?action=list&currentPage=${page.currentPage-1 }')">上一页</a></li>
        <c:choose>
        	<%-- 只要总页数小于10直接显示所有页数 --%>
        	<c:when test="${page.totalPage<10 }">
        		  <c:forEach begin="1" end="${page.totalPage }" var="pc" step="1">
	        	 	<li><a href="javascript:pageFunction('points?action=list&currentPage=${pc }')">${pc }</a></li>
	        	  </c:forEach>
        	</c:when>
        	<%-- 总页数大于10 --%>
        	<c:otherwise>
        ...
        <li><a href="javascript:pageFunction('points?action=list&currentPage=${page.currentPage+1 }')">下一页</a></li>
    </ul>
</div>
				...
				<div class="modal small hide fade" id="myModal" tabindex="-1"
					...
					<div class="modal-body">
						<form id="uploadForm" action="stuImport" method="post"
