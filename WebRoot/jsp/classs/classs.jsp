    ...
    <div class="container-fluid">
                ...
                <div class="sidebar-nav">
                  <div class="nav-header" data-toggle="collapse" data-target="#dashboard-menu">系统管理</div>
                    <ul id="dashboard-menu" class="nav nav-list collapse in">
                        ...
                        <li   class="active" ><a href="classs">班级管理</a></li>
        ...
        <div class="span9">
				...
				<div class="well search-well">
					<form id="searchForm" class=form-inline action="classs"
						method="post">
							<input class="input-xmax" placeholder="班级名" name="name" type="text"
							value="${ classs.id}"> 
						<input class="input-xmax" placeholder="班级名" name="name" type="text"
							value="${ classs.name}"> 
							<input class="input-xmax" placeholder="班级类型" name="name" type="text"
							value="${ classs.type}"> 	
						...
						<tbody>
							<c:forEach items="${list}" var="bean" varStatus="Status">
								<tr>
									...
									<td>${bean.type}</td>
									<td>${bean.openTime}</td>
									<td><a href=" stu?action=toUpdate&id= "><i
											class="icon-pencil"></i></a> <a href="todelete" role="button"
...
<div class="pagination">
    ...
    <ul>
        <li><a href="javascript:pageFunction('classs?action=list&currentPage=${page.currentPage-1 }')">Prev</a></li>
        <c:choose>
        	<%-- 只要总页数小于10直接显示所有页数 --%>
        	<c:when test="${page.totalPage<10 }">
        		  <c:forEach begin="1" end="${page.totalPage }" var="pc" step="1">
	        	 	<li><a href="javascript:pageFunction('classs?action=list&currentPage=${pc }')">${pc }</a></li>
	        	  </c:forEach>
        	</c:when>
        	<%-- 总页数大于10 --%>
        	<c:otherwise>
        ...
        <li><a href="javascript:pageFunction('classs?action=list&currentPage=${page.currentPage+1 }')">Next</a></li>
    </ul>
</div>
